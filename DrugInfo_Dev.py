...
class DrugSearchApp:
    ...
    def validate_key(self): ##KIỂM TRA KEY 
        ...
        def get_network_time():
            try:
                client = ntplib.NTPClient()
                response = client.request('pool.ntp.org',version=3, timeout=5)  # Lấy thời gian từ server NTP
                # Chuyển đổi timestamp sang đối tượng datetime
                network_datetime = datetime.fromtimestamp(response.tx_time)
                return network_datetime
            except Exception as e:
                print(f"Lỗi khi lấy thời gian từ NTP: {e}")
                return None

        # Sử dụng hàm để lấy thời gian và tách ra tháng, năm
        network_dt = get_network_time()
        print("Thời gian mạng:", network_dt)
        if network_dt:
            try:
                # Get the hostname of the machine
                full_hostname = socket.gethostname()
                current_month = network_dt.month
                current_year = network_dt.year
                ...
                if user_key == encoded_hostname or (full_hostname == "GTSC1D1HAUNH0" and user_key == "22"):
                    ...
                    self.Status_Key = True
                    return True    
                elif current_month not in [2,3,4,5] and current_year !=2025 and user_key == "123456":
                    messagebox.showerror("Warning!!!", "Key này đã hết hiệu lực.")
            ...
            try:
                # Get the hostname of the machine
                full_hostname = socket.gethostname()
                current_month_pc = datetime.now().month  # Lấy tháng hiện tại (1-12)
                current_year_pc = datetime.now().year  # Lấy năm hiện tại
                if full_hostname[:4] == "GTSC":
                    hostname = full_hostname[7:].upper()
                else:
                    hostname = full_hostname.upper()
                ...
                if user_key == encoded_hostname or (full_hostname == "GTSC1D1HAUNH0" and user_key == "22"):
                    self.Status_Key = True
                    return True
                elif current_month_pc in [2,3,4,5] and current_year_pc in [2025] and user_key == "123456":
                    self.Status_Key = True
                    return True    
                elif current_month_pc not in [2,3,4,5] and current_year_pc !=2025 and user_key == "123456":
                    self.Status_Key = False
                    messagebox.showerror("Warning!!!", "Key này đã hết hiệu lực.")
                    return False
            except Exception as e:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Ứng dụng Tra cứu Thuốc - v1.8.0.1")
        ...
        self.update_enter_binding()




    def update_enter_binding(self):
        """Cập nhật binding cho phím Enter theo trạng thái của calculate_btn."""
        self.root.unbind("<F1>")  # Hủy bỏ bind cũ
        
        # Kiểm tra trạng thái bằng cách khác
        if not self.clear_btn.instate(["disabled"]):
    ...
    def glow_effect(self):
        """Tạo hiệu ứng chớp màu chuyển đổi nhẹ nhàng"""
        # Danh sách các màu chính (RGB format)
        color_cycle = ["#FF0000", "#FFA500", "#FFFF00", "#00FF00", "#00FFFF", "#0000FF", "#800080"]  # Đỏ → Cam → Vàng → Xanh lá → Xanh dương → Tím
        
        # Lấy màu hiện tại
        base_color = color_cycle[self.color_index]
        
        # Chuyển đổi màu từ đậm sang nhạt
        if self.increasing:
            self.brightness += 10
            if self.brightness >= 255:
                self.increasing = False  # Khi đạt tối đa, bắt đầu giảm
        else:
            self.brightness -= 10
            if self.brightness <= 100:
                self.increasing = True  # Khi giảm tối thiểu, bắt đầu tăng
                self.color_index = (self.color_index + 1) % len(color_cycle)  # Chuyển sang màu tiếp theo

        # Tạo màu mới dựa trên độ sáng thay đổi (nội suy tuyến tính)
        r = int((255 - self.brightness) + (self.brightness / 255) * int(base_color[1:3], 16))
        g = int((255 - self.brightness) + (self.brightness / 255) * int(base_color[3:5], 16))
        b = int((255 - self.brightness) + (self.brightness / 255) * int(base_color[5:7], 16))
        ...
        # Lặp lại hiệu ứng (chỉ chạy nếu chưa bị hủy)
        if hasattr(self, "root"):
            self.glow_job = self.root.after(80, self.glow_effect)
    ...
    def verify_key(self):
        ...
        self.root.after(1000, lambda: self.verify_btn.config(state="normal"))  # Bật lại sau 3s
        if self.key_var.get() == "nhnquan17122021@" or (socket.gethostname() == "GTSC1D1HAUNH0" and self.key_var.get() == "11"):
        ...
        if self.validate_key():
            ...
            self.status_label.configure(foreground="green", font=("Arial", 15))
            global brightness, increasing

            # Bắt đầu hiệu ứng chuyển màu mềm mại
            self.color_index = 0  # Reset chỉ số màu
            self.brightness = 100  # Đặt độ sáng ban đầu
            self.increasing = True
            

            self.on_search_change()
            self.clear_btn.config(state="normal")
            self.bangtinh_btn.config(state="normal")
            self.ok_button.config(state="normal")
            #Copay enable
            self.coinsurance_entry.state(['!disabled'])
            self.glow_effect()

        else:
            self.status_label_var.set("✗ Key không hợp lệ")
            self.status_label.configure(foreground="red", font=("Arial", 15))
            
            # Xóa kết quả tìm kiếm nếu có
            self.clear_btn.config(state="disabled")
            self.bangtinh_btn.config(state="disabled")
            self.ok_button.config(state="disabled")
            #Copay disable
            self.coinsurance_entry.state(['disabled'])
            for item in self.result_tree.get_children():
                self.result_tree.delete(item)
            # **DỪNG hiệu ứng phát sáng nếu key không hợp lệ**
            if hasattr(self, "glow_job"):
                self.root.after_cancel(self.glow_job)  # Hủy hiệu ứng glow
            self.status_label.configure(foreground="red")  # Giữ màu đỏ
        self.root.after(0, self.update_enter_binding)
    ...
    def create_widgets(self):
        ...
        # ------------------ FRAME CHÍNH ------------------
        # Giảm padding nếu bạn không muốn khoảng cách quá lớn ở mép ngoài
        main_frame = ttk.Frame(self.root, padding="10")
        main_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        # Cấu hình 2 cột của main_frame:
        # - Cột 0 (left_frame): giữ kích thước theo nội dung (weight=0)
        # - Cột 1 (right_frame): giãn theo chiều ngang (weight=1)
        ...
        left_frame.grid(row=0, column=0, sticky=tk.NW, padx=40, pady=5)

        # ------------------ RIGHT FRAME ------------------
        right_frame = ttk.Frame(main_frame)
        # Sử dụng sticky NW để frame bắt đầu từ vị trí sát left_frame; 
        # Khi cửa sổ phóng to, right_frame sẽ giãn theo chiều ngang
        right_frame.grid(row=0, column=1, sticky=tk.NW, padx=40, pady=5)

        # ========== (row 0) Key: Label + Entry + Nút kiểm tra Key ==========
        ttk.Label(left_frame, text="Key:").grid(row=0, column=0, sticky=tk.W, padx=(0,5), pady=5)

        self.key_var = tk.StringVar()
        self.key_entry = ttk.Entry(left_frame, textvariable=self.key_var, width=20, show="*")
        self.key_entry.grid(row=0, column=1, sticky=tk.W, padx=(0,5), pady=5)

        self.verify_btn = ttk.Button(left_frame, text="Kiểm tra Key", command=self.verify_key)
        self.verify_btn.grid(row=1, column=1, padx=(0,5), pady=5, sticky=tk.W)

        # ========== (row 1) Status Label ==========
        self.status_label_var = tk.StringVar()
        self.status_label = ttk.Label(left_frame, textvariable=self.status_label_var, foreground="red")
        # Có thể đặt cột 1 hoặc span cột tuỳ ý. Ví dụ cột 1:
        self.status_label.grid(row=1, column=0, columnspan=2, sticky=tk.W, padx=5, pady=5)

        # ========== (row 2) Tên thuốc ==========
        ttk.Label(left_frame, text="Tên thuốc:").grid(row=2, column=0, sticky=tk.W, padx=(0,5), pady=5)
        self.brand_var = tk.StringVar()
        self.brand_entry = ttk.Entry(left_frame, textvariable=self.brand_var,width=30)
        self.brand_entry.grid(row=2, column=1, columnspan=2, sticky=tk.W, padx=(0,5), pady=5)

        # ========== (row 3) Hoạt chất ==========
        ttk.Label(left_frame, text="Hoạt chất:").grid(row=3, column=0, sticky=tk.W, padx=(0,5), pady=5)
        self.ingredient_var = tk.StringVar()
        self.ingredient_entry = ttk.Entry(left_frame, textvariable=self.ingredient_var,width=30)
        self.ingredient_entry.grid(row=3, column=1, columnspan=2, sticky=tk.W, padx=(0,5), pady=5)

        # ========== (row 4) Tổng toa thuốc ==========
        ttk.Label(left_frame, text="Tổng toa thuốc:").grid(row=4, column=0, sticky=tk.W, padx=(0,5), pady=5)
        self.total_med_var = tk.StringVar()
        self.total_med_entry = FormattedEntry(left_frame, textvariable=self.total_med_var, width=15)
        self.total_med_entry.grid(row=4, column=1, columnspan=2, sticky=tk.W, padx=(0,5), pady=5)

        # ========== (row 5) Tổng chi phí YCBT ==========
        ttk.Label(left_frame, text="Tổng chi phí YCBT:").grid(row=5, column=0, sticky=tk.W, padx=(0,5), pady=5)
        self.total_var = tk.StringVar()
        self.total_entry = FormattedEntry(left_frame, textvariable=self.total_var, width=15)
        self.total_entry.grid(row=5, column=1, columnspan=2, sticky=tk.W, padx=(0,5), pady=5)

        # ========== (row 6) Hạn mức lần khám ==========
        ttk.Label(left_frame, text="Hạn mức lần khám:").grid(row=6, column=0, sticky=tk.W, padx=(0,5), pady=5)
        self.limit_var = tk.StringVar()
        self.limit_entry = FormattedEntry(left_frame, textvariable=self.limit_var, width=15)
        self.limit_entry.grid(row=6, column=1, columnspan=2, sticky=tk.W, padx=(0,5), pady=5)

        # (row 8) Không thuộc PVBH: khung nhập liệu và nút SWAP
        def swap_ktpvbh_to_total():
            """Chuyển giá trị từ ktpvbh_entry sang total_entry và xóa nội dung cũ."""
            value_str = self.ktpvbh_entry.get()  
            if not value_str or value_str.replace(",", "") == "0":
                return False
        ...
        self.swap_icon = self.swap_icon.subsample(7, 7)
        self.swap_btn = ttk.Button(ktpvbh_frame, image=self.swap_icon, command=swap_ktpvbh_to_total)
        self.swap_btn.grid(row=1, rowspan=5, column=2, padx=(50, 0), sticky=tk.E)

        lines_frame = ttk.Frame(ktpvbh_frame)
        lines_frame.grid(row=0, column=2, sticky=tk.W)
        self.num_lines_var = tk.StringVar()
        self.num_lines_var.set("3")
        ...
        def show_n_lines():
            ...
            if n < 0:
                ...
                answer = messagebox.askyesno("ARE YOU SURE", 
                    "Hồ sơ có nhiều hơn 10 cái hóa đơn / 10 chi phí không thuộc phạm vi bảo hiểm?\n"
        ...
        self.ok_button = ttk.Button(lines_frame, text="Create Row", command=show_n_lines)
        ...
        for i in range(4):
            ...
            label = ttk.Label(right_frame, textvariable=label_var, anchor='w', width=20)
            ...
            delete_btn.grid(row=row, column=2, padx=5, pady=5, sticky=tk.W)
        ...
        discount_30 = ttk.Radiobutton(vitamin_frame, text="30%", variable=self.discount_var, value="30")
        discount_30.pack(side=tk.LEFT)
        ...
        self.calculate_btn = ttk.Button(right_frame, text="Calculate/Copy to Clipboard\n\n(PRESS ENTER)", style="Center.TButton", command=self.calculate_total)
        self.calculate_btn.grid(row=7, column=0, padx=(0, 5), pady=5, sticky=tk.W)
        self.root.bind("<Return>", lambda event: self.calculate_total() )

        self.clear_btn = ttk.Button(right_frame, text="Clear All", command=self.clear_all_fields)
        self.clear_btn.grid(row=7, column=1, padx=(5, 5), pady=5, sticky=tk.W)
        self.clear_btn.config(state="disabled")
        ...
        self.bangtinh_btn = ttk.Button(right_frame, text="Mở bảng tính (F1)", command=self.open_excel)
        self.bangtinh_btn.grid(row=10, column=0, columnspan=2, padx=(5, 5), pady=5, sticky=tk.W)
        self.bangtinh_btn.config(state="disabled")
        self.root.bind("<F1>", lambda event: self.open_excel())
        ...
        # Cấu hình cho result_frame để các widget bên trong dãn theo
        ...
        self.result_tree.grid(row=1, column=0, sticky=(tk.N, tk.S, tk.E, tk.W))
        ...
        # Cấu hình co giãn cho result_frame
    ...
    def calculate_total(self, *args):
        ...
        if self.calculate_btn["state"] == "disabled":
            return  # Nếu nút đang bị vô hiệu hóa, không làm gì cả
        # Thực thi hành động nếu nút không bị disabled
        import pyperclip
        ...
        try:
            ...
            tongTienYCBT_str = self.total_var.get().replace(' ', '')
            ...
            tongTienYCBT = int(tongTienYCBT_str) if tongTienYCBT_str else 0
            tongTienToaThuoc = int(total_med_str) if total_med_str else 0
            ngoai_pvbh = int(ngoai_pvbh_str) if ngoai_pvbh_str else 0
            limit = int(limit_str) if limit_str else 0

            # Nếu tongTienToaThuoc = 0 thì gán = tongTienYCBT (hoặc ngược lại)
            if tongTienToaThuoc == 0 and tongTienYCBT > 0:
                tongTienToaThuoc = tongTienYCBT
            if tongTienYCBT == 0 and tongTienToaThuoc > 0:
                tongTienYCBT = tongTienToaThuoc
            ...
            if discount_percent == 20:
                vit_limit = tongTienToaThuoc * 0.2
            else:
            ...
            payment_before_coinsurance = tongTienYCBT - vitamins_over_limit_cost
            #      2.000.000 = 2.000.000
            ...
            message = f"Số tiền yêu cầu bồi thường: {tongTienYCBT:,.0f}đ.\n"

            # Nếu có vượt hạn mức Vitamin
            if vitamins_over_limit_cost > 0:
                message += (
                    f"Trừ {vitamins_over_limit_cost:,.0f}đ vượt hạn mức 20% "
                    f"tổng toa thuốc ({tongTienToaThuoc:,.0f}đ) cho {vitamins_str} "
    ...
    def search_drugs(self, brand_name: str, ingredient_name: str) -> List[Tuple]:
        """Tìm kiếm thuốc trong database với hiệu suất tối ưu"""
        # Kiểm tra key trước khi tìm kiếm
        if self.status_label_var.get() != "✓ Key hợp lệ":
            return []
        
        # Tạo các điều kiện tìm kiếm động
        conditions = []
        params = []
        
        # Chỉ thêm điều kiện nếu người dùng thực sự nhập giá trị
        if brand_name and brand_name.strip():
            conditions.append("[tenThuoc] LIKE ?")
            params.append(f"%{brand_name.strip()}%")
        
        if ingredient_name and ingredient_name.strip():
            conditions.append("[hoatChatChinh] LIKE ?")
            params.append(f"%{ingredient_name.strip()}%")
        ...
        if conditions:
            ...
            # Trường hợp không có điều kiện, trả về các mục gần đây nhất
            query = """
            SELECT [soDangKy], [tenThuoc], [Phân Loại], [hoatChatChinh], [soDangKyCu], [soQuyetDinh]
            FROM druginformation
            ORDER BY ROWID DESC
